...
| # | Item | Owner | Notes |
|---|---|---|---|
| 13 | Provision real production email for Titan: confirm which domain is verified on the already-active Resend account (`RESEND_API_KEY` is already in this repository's GitHub Actions secrets) and add a matching `EMAIL_FROM` value there too, then run the `Titan Provision Environment` workflow (`titan-provision.yml`) for `production` | Founder/Business owner (the `EMAIL_FROM` value/domain decision) + Engineering (the workflow run itself) | `RISK_REGISTER.md` risk 16a. Code is complete and tested — see `docs/titan/DECISION_LOG.md`'s 2026-07-23 Resend entry (and its same-day correction). Nothing left to build; this is one real value + a deploy action |
