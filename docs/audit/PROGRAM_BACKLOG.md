...
| 17 | Trigger `titan-deploy.yml` (`target_environment: production`) from a context with real GitHub Actions dispatch permission | Founder/Business owner, or any session with `actions:write` | `docs/titan/DECISION_LOG.md`'s 2026-07-23 "pushed to `main`" entry — this session's own GitHub integration got `403 Resource not accessible by integration` attempting the dispatch, the same class of permission gate `RELEASE_CANDIDATE_GUIDE.md` already hit trying to push a git tag. `main` is current; nothing further blocks a real deploy attempt except who's allowed to press the button. Will also need real `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` repository secrets, which have never existed for this project |
...
| Titan `main`/deploy decision (former item 16) | Founder chose "push to main AND deploy," explicitly accepting the live-Razorpay-verification gap named above. `main` pushed as a clean fast-forward (`c393580..89d2903`). The deploy half is blocked, not skipped — see item 17 above for why |
