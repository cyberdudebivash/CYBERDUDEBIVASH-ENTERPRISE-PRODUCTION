...
| 11 | `compliance.html`, `dark-web-monitor.html`, `vciso.html` have no footer at all | Engineering | `RISK_REGISTER.md` risk 14. Lower priority than the now-fixed missing Terms/Copyright pages since these 3 pages have no legal links to be broken — but still inconsistent with every other static page |
