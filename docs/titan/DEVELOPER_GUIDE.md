...
`.dev.vars.example` also has `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` placeholders (blank by default, same convention as the OAuth provider vars above them) — this project has never had a real Razorpay account in any environment, so leaving them blank is the normal, expected local-dev state: `worker.ts` only constructs `razorpayCredentials` when both are present, so the DPDP Scanner's Razorpay routes correctly return a real 503 "not configured" rather than attempting a real (and failing) call to Razorpay's live API.
...
## Deploying Titan (PRD-1)

Full detail lives in `DEPLOYMENT_GUIDE.md`/`ENVIRONMENT_GUIDE.md`/`DISASTER_RECOVERY.md` — this section is the short, developer-facing version: what exists, what's safe to run locally, and the guardrails for extending it.

```bash
cd titan/packages/platform
npm run config:check:staging          # real, safe, no credentials needed
npm run secrets:validate:local        # checks .dev.vars, real, safe
npm run deploy:dry-run:staging        # real wrangler dry-run, no credentials needed
npm run db:backup:local               # real local D1 export
npm run smoke-test -- --base-url http://localhost:8787   # against a running `wrangler dev`
```

- **`wrangler.toml` has named `[env.staging]`/`[env.production]` blocks now — they do not inherit the default (local-dev) block's `[[d1_databases]]`/`[vars]`.** If you add a new binding or var that a real deployment needs, add it to *all three* places (default, staging, production) or it silently won't exist in whichever block you forgot — verify with `wrangler deploy --dry-run --env <name>` after any change, the same real, credential-free check this phase used throughout.
- **Never put a real secret value in `wrangler.toml`.** `[vars]` blocks are plaintext, committed config — `AUTH_SECRET` and friends only ever belong in `.dev.vars` locally or `wrangler secret put --env <tier>` for a real deployment. `scripts/validate-secrets.mjs` exists specifically to catch a missing secret *name* without ever needing to see a value.
- **`titan-deploy.yml` (`.github/workflows/`) is manual-only (`workflow_dispatch`) on purpose.** Don't add a `push`/`schedule` trigger to it without a real go-ahead — this project has no Cloudflare credentials configured anywhere, so an automatic trigger would just be a workflow that fails on every run, and the day credentials do exist, an accidental push should still never deploy without a human choosing to.
- **`config/validateEnv.ts`'s `validateProductionConfig` runs on every request, not once at startup** — Workers has no separate startup phase with `env` bindings available (they only exist inside `fetch(request, env, ctx)`). If you add a check here, keep it a pure, synchronous, no-I/O function — the same reason this one is cheap enough to run unconditionally on every request rather than needing to be cached or debounced.
...
## Enterprise Security & Compliance usage (SEC-1)

Full detail lives in `SECURITY_ARCHITECTURE.md`/`THREAT_MODEL.md`/`COMPLIANCE_GUIDE.md` — this section is the short, developer-facing version.

- **The role model is one enum, three values (`member`/`admin`/`owner`), plus a Platform Administrator sentinel convention — not four roles.** If you write or update security documentation, use this precise framing (`SECURITY_ARCHITECTURE.md`'s authorization flow section) rather than repeating an imprecise "4 roles" shorthand.
- **Every state-changing route's CSRF check (`isTrustedOrigin`) and rate limit (`checkRateLimit`) live inline in `router.ts`, in that order — CSRF first, then rate limit, then auth/authz.** If you add a new write route, follow this exact ordering (see any of the 7 rate-limited routes for the pattern) rather than inventing a new one.
- **`STRICT_CSP`/`authPagesCsp` (`http/finalizeResponse.ts`) both set `frame-ancestors 'none'` explicitly — don't remove it and don't assume `default-src` covers it.** CSP3's `frame-ancestors` does not fall back to `default-src`; if you add a third CSP variant for a new route type, it needs its own explicit `frame-ancestors` too.
- **Don't add rate limiting to `GET /health`/`GET /health/ready` or any Platform-Administrator-only `GET` route without a real, evaluated reason.** Both are deliberate exceptions — health/readiness need to tolerate frequent legitimate polling once deployed, and rate-limiting an operator's own dashboard would undermine OPS-1's own purpose (`SECURITY_ARCHITECTURE.md`'s rate limiting section has the full reasoning).
- **Before running `npm audit fix --force`, read `THREAT_MODEL.md`'s SEC-1-01 finding first.** It looks like a routine fix; it is actually a ~97-minor-version `wrangler` downgrade. `npm audit fix --force --dry-run` itself is not safe to assume is non-mutating either — it removed real packages from `node_modules` when run this phase, despite the flag name (recovered with a clean `npm ci`).
...
- Don't build Billing/Payment Gateway Processing, PCI card storage, Accounting, Tax Engines, a Marketplace, Affiliate Programs, CRM, ERP, an actual Cloudflare Production deployment, a Release Candidate, or General Availability without a real go-ahead — EAP-1 deliberately built only the shell and one module (Dashboard); EAP-2 through EAP-8 each deliberately built exactly one more (Lead Intelligence, Assessment Center, Organization Management, Enterprise Identity & User Management, Enterprise Audit Center, Enterprise Operations Center, Enterprise Reporting & Analytics) — every module the original EAP-1 brief named, plus this roadmap-recommended eighth one; CPP-1 then built the first customer-facing module (the Enterprise Customer Portal); COM-1 then built the commercial/entitlement layer (Subscriptions, Licenses, Entitlements) on top of all nine — explicitly *not* the payment gateway logic that would actually charge a card; PRD-1 then built the production deployment *automation* (environments, CI/CD, release scripts, disaster recovery) — explicitly *not* an actual deployment, since no Cloudflare account has ever existed to deploy to. The rest are sequenced, not forgotten (`ROADMAP.md`).
...
- Don't run `npm audit fix --force` to resolve the `sharp`/`miniflare`/`wrangler` finding (`THREAT_MODEL.md`'s SEC-1-01) without first re-checking whether a newer `wrangler`/`miniflare` release has resolved it upstream — the proposed fix as of SEC-1 is a real downgrade to `wrangler@4.15.2` from `4.112.0`, not a safe patch bump. Don't add rate limiting to the seven remaining Platform-Administrator-only write routes, or to any Platform-Administrator-only `GET` route, without a real, evaluated reason — `SECURITY_ARCHITECTURE.md`'s rate limiting section has the full reasoning for why this phase deliberately left them as they were. Don't remove `frame-ancestors 'none'` from either CSP variant (`http/finalizeResponse.ts`) — it's real, verified defense in depth, not a redundant addition.
