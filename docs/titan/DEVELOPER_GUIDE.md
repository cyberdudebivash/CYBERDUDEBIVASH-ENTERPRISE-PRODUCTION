...
titan/
  apps/
    web/                  # @titan/web — the application shell (routing, layout, error handling)
      ...
      src/features/admin/            # the authenticated admin app (/admin, EAP-1) — auth/, layout/, dashboard/, leads/ (EAP-2), assessments/ (EAP-3), organizations/ (EAP-4), users/ (EAP-5), audit/ (EAP-6), operations/ (EAP-7), reporting/ (EAP-8), uses @titan/design-system
  packages/
...
- Don't build a Customer Portal or any commercial/billing/licensing surface as a full module without a real go-ahead for that EAP phase — EAP-1 deliberately built only the shell and one module (Dashboard); EAP-2 deliberately built exactly one more (Lead Intelligence); EAP-3 deliberately built exactly one more (Assessment Center); EAP-4 deliberately built exactly one more (Organization Management); EAP-5 deliberately built exactly one more (Enterprise Identity & User Management); EAP-6 deliberately built exactly one more (Enterprise Audit Center); EAP-7 deliberately built exactly one more (Enterprise Operations Center); EAP-8 deliberately built exactly one more (Enterprise Reporting & Analytics) — every module the original EAP-1 brief named, plus this roadmap-recommended eighth one, are now real; the rest are sequenced, not forgotten (`ROADMAP.md`).
