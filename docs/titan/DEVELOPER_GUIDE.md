...
- Don't build a Reporting & Analytics module, a Customer Portal, or any commercial/billing/licensing surface as a full module without a real go-ahead for that EAP phase — EAP-1 deliberately built only the shell and one module (Dashboard); EAP-2 deliberately built exactly one more (Lead Intelligence); EAP-3 deliberately built exactly one more (Assessment Center); EAP-4 deliberately built exactly one more (Organization Management); EAP-5 deliberately built exactly one more (Enterprise Identity & User Management); EAP-6 deliberately built exactly one more (Enterprise Audit Center); EAP-7 deliberately built exactly one more (Enterprise Operations Center); the rest are sequenced, not forgotten (`ROADMAP.md`).
