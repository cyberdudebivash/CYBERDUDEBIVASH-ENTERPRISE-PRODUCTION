...
- Don't build an Audit Center or an Operations Center (or a Customer Portal, or any commercial/billing/licensing surface) as full modules without a real go-ahead for that EAP phase — EAP-1 deliberately built only the shell and one module (Dashboard); EAP-2 deliberately built exactly one more (Lead Intelligence); EAP-3 deliberately built exactly one more (Assessment Center); EAP-4 deliberately built exactly one more (Organization Management); EAP-5 deliberately built exactly one more (Enterprise Identity & User Management); the rest are sequenced, not forgotten (`ROADMAP.md`).
