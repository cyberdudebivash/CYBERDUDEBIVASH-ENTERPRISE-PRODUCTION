...
| I — Admin Portal | Dashboard, assessments, companies, leads, reports | 🟡 **Started — EAP-1 Phase 1.** Application shell (session lifecycle, protected routes, role-aware nav, reusable UI framework) plus one complete module: Dashboard (executive overview, org/lead/assessment metrics, audit summary, platform health). Assessments/companies/leads *management* (create/edit/search/detail views), and reports, are not built — see the EAP-1 section below |
...
| L — Quality | Full build/typecheck/lint/test/a11y/perf/security/regression pass | ✅ Passes for everything that exists — 300 tests across 4 packages (up from 119 at Stage 4's start, 222 as of ERP-1, 244 as of the Security Release Blocker Sprint, +56 for EAP-1 Phase 1), full CI green, real `wrangler dev` + real Chromium browser verification (both manual and two committed Playwright E2E specs, 4 tests total) |
...
## EAP-1: the Admin Portal as an explicitly phased program, not one pass

The "EAP-1 — Enterprise Administration Platform" prompt requested a full admin application in one pass: shell, navigation, dashboard, Lead/Assessment/Organization/User Management, an Audit Center, and an Operations Center. Given the choice between shallow progress across all of that or real depth on a scoped subset, the user directed this to be built as a phased program and scoped this session to Phase 1 only — full reasoning in `DECISION_LOG.md`'s EAP-1 entry.

### Phase 1 — Enterprise Administration Foundation (this pass)

**Done:** the authenticated application shell (session lifecycle via real Auth.js sign-in/sign-out, protected-route middleware, role-aware navigation built for pluggability, reusable admin UI framework — `MetricCard`/`Panel`), and one complete module: **Dashboard** (executive overview, organization/lead/assessment metrics, risk-level breakdown, recent activity, audit summary, platform health/system status). Four new read endpoints (`GET /api/me`, `GET /api/organizations`, `GET /api/assessments` list, `GET /api/audit`) back it, reusing the existing Platform Administrator role — no new authorization concept. Found and fixed one real, non-obvious bug that only real-browser verification could catch: CSP's `form-action` directive also restricts the redirect a form submission causes, which silently broke cross-origin sign-out until `authPagesCsp` widened it. Full detail: `ARCHITECTURE.md`'s "Admin Application architecture" section, `DECISION_LOG.md`'s EAP-1 entry, `PLATFORM_FOUNDATION.md`'s EAP-1 Phase 1 section.

**Explicitly not done this phase** (deferred to later phases, not forgotten, not stubbed ahead of time): Lead Management, Assessment Management, Organization Management, User Management, an Audit Center, and an Operations Center as full modules — no create/edit/delete, no per-record detail views, no search/filter beyond what the Dashboard's own summary needed. The shell is architected so each plugs in without rework: `adminNavItems` takes the caller's real role and is where a new module's nav entry gets added; `AdminLayout`'s `<Outlet/>` is the mount point new module routes nest under; `useDashboardData`'s per-section `SectionState` (loading/ready/forbidden/error) pattern is meant to be reused by each new module's own data-loading hook rather than reinvented.

### Recommended next EAP phase

**Phase 2 — Lead Management**, as the natural next module: `GET /api/leads` and the Platform Administrator authorization it needs already exist and are already exercised by the Dashboard's metrics; a management UI (list with real search/filter, per-lead detail view) is the smallest real next slice of product value, and the first UI consumer of data this platform has been collecting since Stage 4. Assessment Management is the next-most-ready candidate after that (`GET /api/assessments` list and `GET /api/assessments/:id` both already exist), followed by Organization Management, User Management, the Audit Center, and the Operations Center — roughly in order of how much of their backing API already exists versus how much net-new API surface each would need. This is a recommendation for sequencing, not a decision already made — the same way `ARCHITECTURE.md`'s open decisions are recommendations, not commitments.

## What Stage 5 / the next pass needs

1. **EAP-1 Phase 2 (Lead Management)** — see above.
2. **Organization-scoped filtering for `GET /api/leads`/`GET /api/organizations`/`GET /api/assessments`/`GET /api/audit`**, if an organization's own admin (not just a Platform Administrator) should ever see their own organization's data. Needs each repository's query layer to support it — not added speculatively ahead of a management UI needing it (`SECURITY_GUIDE.md`'s "Known, accepted gaps").
