...
| L — Quality | Full build/typecheck/lint/test/a11y/perf/security/regression pass | ✅ Passes for everything that exists — 244 tests across 4 packages (up from 119 at Stage 4's start, 222 as of ERP-1), full CI green, real `wrangler dev` + real Chromium browser verification (both manual and a committed Playwright E2E suite) |
...
1. **Admin Portal (I).** Now has somewhere real to read from *and* a real authorization mechanism already wired to the routes it needs (Stage 4/RC1 built the repositories and API; this sprint fixed and wired the authorization). The natural next product surface, and the first real caller of `GET /api/leads`/`GET /api/assessments/:id` outside manual/test verification.
2. **Organization-scoped filtering for `GET /api/leads`**, if an organization's own admin (not just a Platform Administrator) should ever see their own leads. Needs `leadRepository`'s query layer to support it — not added speculatively ahead of Admin Portal needing it (`SECURITY_GUIDE.md`'s "Known, accepted gaps").
