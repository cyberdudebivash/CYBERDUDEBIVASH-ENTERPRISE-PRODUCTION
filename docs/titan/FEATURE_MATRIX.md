...
Admin portal, customer portal, AI insights, booking, payments, real (deployed) Cloudflare infrastructure, email delivery (Auth.js's Email provider logs instead of sending), enterprise SSO, organization-scoped query filtering for `GET /api/leads` (still Platform-Administrator-only — `SECURITY_GUIDE.md`'s "Known, accepted gaps"), a self-service way to grant the Platform Administrator role (provisioned via direct SQL only, `OPERATIONAL_RUNBOOK.md`) — all out of scope for this stage or explicitly deferred (`ROADMAP.md`, `DECISION_LOG.md`). Nothing above should be read as implying progress on anything not explicitly listed.
