...
| `npm install`/`npm ci` reports "3 high severity vulnerabilities" | Expected, not a new problem introduced by your own install (SEC-1) — a real, known, tracked finding in `sharp`, a transitive devDependency of `miniflare`/`wrangler`, never reachable from application code | Don't run `npm audit fix --force` to "clean this up" — it proposes a real `wrangler` downgrade (`4.112.0` → `4.15.2`), not a safe patch. See `THREAT_MODEL.md`'s SEC-1-01 for the full reasoning; re-run plain `npm audit` (no `fix`) periodically to check for a real, non-breaking upstream fix instead |

## What this runbook does not cover

**A real deployment.** `DEPLOYMENT_GUIDE.md` (PRD-1) now documents the real, structurally-verified deployment *automation* — named environments, a CI/CD pipeline, release-automation scripts, all actually run against real local state — and `DISASTER_RECOVERY.md`/`ENVIRONMENT_GUIDE.md` cover backup/recovery and environment configuration in the same evidence-only way this document does for local operation. None of the three describes a real deployment having happened, because one hasn't — no Cloudflare account or credentials have existed in any environment this project has run in. This document, and those three, only cover what has actually been run and verified. **GA-1** re-ran every verification command in this document fresh (`npm run ci`, the full Playwright suite, `wrangler deploy --dry-run`) as part of labeling the current commit `0.1.0-rc.1` — see `RELEASE_CANDIDATE_GUIDE.md` for the consolidated, cross-phase result and `GLOBAL_LAUNCH_GUIDE.md` for what has to be true before any of this document's local-only procedures have a real, deployed counterpart to run against.
