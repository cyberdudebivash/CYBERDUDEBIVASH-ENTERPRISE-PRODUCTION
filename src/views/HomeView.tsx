    ...
    <div className="p-4 md:p-8 space-y-8 max-w-7xl mx-auto w-full animate-fade-in">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-2">
          <div className="space-y-0.5">
            ...
            <p className="text-[10px] text-slate-300 font-mono">Fully unified AI native network threat assessment, incident emulation, and real-time mitigation playbooks.</p>
          </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-2">
          <div className="space-y-0.5">
            ...
            <p className="text-[10px] text-slate-300 font-mono">Direct secure gateways to active corporate sub-nets &amp; public cloud channels.</p>
          </div>
          <div className="text-[9px] font-mono text-slate-300 flex items-center gap-2">
              ...
              >
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-1">
                    <span className="text-[9px] font-mono text-slate-300 uppercase tracking-wider">{portal.tagline}</span>
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-lg overflow-hidden animate-fade-in">
          <div className="bg-[#0c1117] px-4 py-2 border-b border-slate-800 flex items-center justify-between">
            ...
            <button
              ...
              className="text-slate-300 hover:text-white font-mono text-[10px] uppercase tracking-wider cursor-pointer"
            >
              [Close Terminal]
            </button>
          </div>
          <div className="p-4 font-mono text-[11px] text-slate-300 space-y-1 bg-black/90 select-text max-h-48 overflow-y-auto leading-relaxed">
      ...
      <div className="bg-[#0c1117]/60 border border-slate-800/80 rounded-lg p-5 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-900 pb-3">
          <div className="space-y-0.5">
            ...
            <p className="text-[10px] text-slate-300 font-mono">Autonomous media outlets, freelancing agency registries, and verified developer coordinates.</p>
          </div>
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[9px] font-mono text-slate-400 block uppercase tracking-tight shrink-0">{prof.category}</span>
                </div>
                <h5 className="text-[11px] font-extrabold text-slate-200 truncate">{prof.username}</h5>
                <span className="text-[9px] font-mono text-slate-300 block leading-snug">{prof.metric}</span>
              </div>
      ...
      <div className="bg-slate-900/50 border border-slate-800/80 rounded-lg p-6 space-y-4">
        <div className="text-center space-y-1">
          ...
          <p className="text-[11px] text-slate-300">{COMPLIANCE_DISCLOSURE} <button onClick={() => onNavigate("about")} className="underline hover:text-cyan-400 cursor-pointer">See our compliance statement.</button></p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 bg-[#0c1117]/80 border border-slate-800 p-6 rounded-lg">
        <div className="lg:col-span-7 space-y-4">
          <div className="border-l-4 border-cyan-500 pl-3">
            ...
            <p className="text-[10px] text-slate-300 font-mono">Government of India official incorporation registry details for CyberDudeBivash Pvt Ltd.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-mono">
            <div className="bg-slate-950 p-3 rounded border border-slate-900/60 space-y-1">
              <span className="text-[9px] text-slate-300 uppercase tracking-wider block">Legal Name (Founder)</span>
              <span className="text-slate-200 font-bold block">{CORPORATE_REGISTRATION.legalName}</span>
            </div>
            <div className="bg-slate-950 p-3 rounded border border-slate-900/60 space-y-1">
              <span className="text-[9px] text-slate-300 uppercase tracking-wider block">Trade / Brand Entity</span>
              <span className="text-cyan-400 font-bold block">{CORPORATE_REGISTRATION.tradeName}</span>
            </div>
            <div className="bg-slate-950 p-3 rounded border border-slate-900/60 space-y-1">
              <span className="text-[9px] text-slate-300 uppercase tracking-wider block">Permanent Account Number (PAN)</span>
              <span className="text-amber-500 font-bold block">{CORPORATE_REGISTRATION.pan}</span>
            </div>
            <div className="bg-slate-950 p-3 rounded border border-slate-900/60 space-y-1">
              <span className="text-[9px] text-slate-300 uppercase tracking-wider block">Government GSTIN ID</span>
            ...
            <div className="bg-slate-950 p-3 rounded border border-slate-900/60 space-y-1 sm:col-span-2">
              <span className="text-[9px] text-slate-300 uppercase tracking-wider block">Registered HQ Physical Address</span>
        ...
        <div className="lg:col-span-5 bg-slate-950 border border-slate-900 rounded-lg p-5 flex flex-col justify-between space-y-4">
          ...
          <div className="bg-black border border-slate-800 rounded p-4 space-y-3">
            <div className="space-y-1">
              <span className="text-[9px] text-slate-300 uppercase tracking-widest block">GSTIN to search</span>
            ...
            <p className="text-[9px] text-slate-300 font-sans leading-snug">
      ...
      <div className="space-y-4">
        <div className="border-l-4 border-cyan-500 pl-3">
          ...
          <p className="text-[11px] text-slate-300">Directly download production-ready compliance assets, SIGMA rules, and response playbooks.</p>
        </div>
            ...
            return (
              <div key={p.id} className="bg-[#0c1117] border border-slate-800 p-4 rounded-lg flex flex-col justify-between space-y-3 hover:border-slate-700 transition-all">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-slate-300 leading-snug">{p.desc}</p>
                </div>
                <button
